...
lojas_selecionadas = st.sidebar.multiselect('Filtrar Loja(s)', options=lojas_do_grupo, default=lojas_do_grupo)
...
with col1:
    data_inicio = st.date_input("De",  value=data_inicio_default, min_value=data_min, max_value=data_max)
with col2:
    data_fim = st.date_input("Até", value=data_fim_default, min_value=data_min, max_value=data_max)
...
df_metricas_vendedor, df_metricas_produto, df_metricas_temporais, lista_consultores = load_data(
    lojas_configs, data_inicio, data_fim
)
...
plot_height = st.sidebar.slider('Ajustar Altura dos Gráficos', 250, 500, 350)


def format_brl(valor):
    return f"R$ {valor:,.2f}".replace(',', 'X').replace('.', ',').replace('X', '.')

st.title('Painel de Indicadores de Vendas POWERX')
...
col1.metric("Sell In Total (R$)", format_brl(kpis_chave.get('Sell In Total (R$)', 0)))
col2.metric("Ticket Médio (R$)",  format_brl(kpis_chave.get('Ticket Médio (R$)', 0)))
col3.metric("Total de Produtos",  f"{int(kpis_chave.get('Total de Produtos', 0)):,} un.")
...
st.subheader('Sell In por Consultor (R$)')

col_sell_in, col_info_sell_in = st.columns([2, 1])

with col_sell_in:
    if not df_sell_in_consultor.empty:
        fig_sell_in = create_sell_in_chart(df_sell_in_consultor, consultor_selecionado, plot_height)
        st.plotly_chart(fig_sell_in, use_container_width=True)
    else:
        st.info("Sem dados de Sell In disponíveis")

with col_info_sell_in:
    st.markdown("##### Sell In por Consultor")
    st.info(
        "Valor total em R$ gerado por cada Consultor Técnico "
        "nas lojas selecionadas no período."
    )
    df_sell_in_filtrado = df_sell_in_consultor[df_sell_in_consultor['Consultor'].isin(consultor_selecionado)]
    if not df_sell_in_filtrado.empty:
        lider = df_sell_in_filtrado.sort_values('Venda_RS', ascending=False).iloc[0]
        st.metric("Maior Sell In", lider['Consultor'], format_brl(lider['Venda_RS']))
...
with col_penetracao:
    if not df_metricas_produto.empty:
        fig_penetracao = create_penetracao_chart(df_metricas_produto[['Descricao', 'Penetracao_Produto']].copy())
...
with col_info:
    ...
    st.info("Distribuição de produtos vendidos agregada por todas as lojas selecionadas.")
    if not df_metricas_produto.empty:
        produto_lider = df_metricas_produto.loc[df_metricas_produto['Penetracao_Produto'].idxmax()]
        st.metric("Produto Líder", produto_lider['Descricao'], f"{produto_lider['Penetracao_Produto']:.2f}%")
...
if not df_metricas_temporais.empty:
    ...
    with col_info2:
        ...
        st.metric("Loja Destaque (Sell In)", loja_top, format_brl(faturamento_top))
else:
    st.info("Sem dados")
...
if st.secrets.get('settings', {}).get('debug_mode', False):
    with st.expander("Debug: Métricas Detalhadas", expanded=False):
        ...
        st.write("### Sell In por Consultor (R$)")
        st.dataframe(df_sell_in_consultor)
